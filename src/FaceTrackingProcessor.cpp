...
void FaceTrackingProcessor::Process(HWND dialogWindow)
{
    ...
    if (FaceTrackingUtilities::GetCheckedModule(dialogWindow))
    {
		...
		if (FaceTrackingUtilities::IsModuleSelected(dialogWindow, IDC_LINE)){
			config->landmarks.isEnabled = FaceTrackingUtilities::IsModuleSelected(dialogWindow, IDC_LINE);
		}
