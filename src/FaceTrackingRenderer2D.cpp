...
void FaceTrackingRenderer2D::DrawGraphics(PXCFaceData* faceOutput)
{
	...
	for (int i = 0; i < numFaces; ++i) 
	{
		...
		if (FaceTrackingUtilities::IsModuleSelected(m_window, IDC_LINE) && trackedFace->QueryDetection() != NULL)
...
void FaceTrackingRenderer2D::DrawBitmap(PXCCapture::Sample* sample, bool ir)
{
	...
	if (image->AcquireAccess(PXCImage::ACCESS_READ, PXCImage::PIXEL_FORMAT_RGB32, &data) >= PXC_STATUS_NO_ERROR)
	{
		...
		//		// âÊëúÇÃÉ`ÉÉÉlÉãêîï™ÇæÇØÉãÅ[ÉvÅBîíçïÇÃèÍçáÇÕ1âÒÅAÉJÉâÅ[ÇÃèÍçáÇÕ3âÒÅ@Å@Å@Å@Å@
...
//äÁó÷äsÇ»Ç«ÇÃì_ï`âÊ
void FaceTrackingRenderer2D::DrawLine(PXCFaceData::Face* trackedFace){
